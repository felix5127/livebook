...
export default function HomePage() {
  ...
  const handleDeleteNotebook = (e: React.MouseEvent, notebookId: string) => {
    ...
    setDeleteModal({
      show: true,
      notebookId,
      notebookTitle: notebook?.title || '笔记本'
    });
  };

  const confirmDelete = () => {
    setNotebooks(prev => prev.filter(notebook => notebook.id !== deleteModal.notebookId));
    setDeleteModal({ show: false, notebookId: '', notebookTitle: '' });
  };
